...
const UpdateArticle: React.FC<{action_url: string}> = ({action_url}) => {
  const { record } = useLocation().state as { record: ArticleType };
  ...
  const CreateNew = async () => {
    ...
    console.log(data);
    const response = await api.patch(`${action_url}/update/${record._id}`, data);
    // Quay lại lúc đâu
    ...
    <>
      <Form layout="vertical" form={form} onFinish={CreateNew}
            initialValues={{ 
              ...
              outstand: record.outstand,
              createdAt: dayjs.utc(record.createdAt),
              introduction: record.introduction,
            }}>
          <div style={{display: 'flex', justifyContent: 'flex-end', gap: 10, marginBottom: 20}}>
            <Button onClick={() => navigate(action_url)}>Cancel</Button>
          ...
          <Row gutter={16} style={{marginBottom: 10}}>
            <Col span={24}>
              <Form.Item name="content" label="Nội dung">
                <TextEditor editorRef={editorRef} defaultValue={record.content}/>
              </Form.Item>
            </Col>
          </Row>
      </Form>
